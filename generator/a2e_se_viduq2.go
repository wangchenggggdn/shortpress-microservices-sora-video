...
type SETaskStatus struct {
	InputImage      string    `json:"input_image"`      // 用户传入的原图（首帧）
	ImageTaskID     string    `json:"image_task_id"`    // A2E 图生图任务 ID
	TailImageURL    string    `json:"tail_image_url"`   // A2E 生成的尾帧图
	VideoTaskID     string    `json:"video_task_id"`    // ViduQ2 视频任务 ID
	ImageParameters string    `json:"image_parameters"` // A2E 参数
	VideoParameters string    `json:"video_parameters"` // ViduQ2 参数
	Status          string    `json:"status"`           // pending, generating_tail, generating_video, completed, failed
...
type a2eSEImageGenerateRequest struct {
...
type a2eSEImageGenerateResponse struct {
	Code int `json:"code"`
	Data []struct {
		Id string `json:"_id"`
	} `json:"data"`
...
type a2eSEImageQueryResponse struct {
	Code int `json:"code"`
	Data struct {
		Id            string   `json:"_id"`
...
type viduq2SEVideoGenerateResponse struct {
	Code int `json:"code"`
	Data struct {
...
type viduq2SEVideoQueryResponse struct {
	Code int `json:"code"`
	Data struct {
...
func NewA2eSEViduQ2() *A2eSEViduQ2 {
	// 初始化 Redis 客户端
...
func (v *A2eSEViduQ2) Generate(args map[string]any) (string, error) {
	ctx := context.Background()

	// 打印用户传入的所有参数
	argsJSON, _ := json.Marshal(args)
	log.Printf("[A2eSEViduQ2] 用户传入参数: %s", string(argsJSON))
	...
	if inputImages == "" {
		return "", fmt.Errorf("缺少 input_images 参数（原图/首帧）")
	}

	log.Printf("[A2eSEViduQ2] 用户提供的原图（首帧）: %s", inputImages)
	...
	log.Printf("[A2eSEViduQ2] 模板 %s (%s)", tmpl.TemplateID, tmpl.Type)

	// 生成任务ID
	taskID := fmt.Sprintf("a2e_se_viduq2_%d", time.Now().UnixNano())

	// 注入 input_images 到 ImageParameters
	...
	if err := json.Unmarshal([]byte(tmpl.ImageParameters), &imgParams); err == nil {
		...
		if newParams, err := json.Marshal(imgParams); err == nil {
			...
			log.Printf("[A2eSEViduQ2] 已注入 input_images 到 ImageParameters (数组格式)")
		}
	}
	...
	if err := v.redis.Set(ctx, taskKey, statusJSON, 24*time.Hour).Err(); err != nil {
		log.Printf("[A2eSEViduQ2] Redis 保存失败: %v", err)
	...
	log.Printf("[A2eSEViduQ2] 创建任务 %s", taskID)

	return taskID, nil
}

// processTask 异步处理任务
func (v *A2eSEViduQ2) processTask(taskID string) {
	ctx := context.Background()
	...
	if err != nil {
		log.Printf("[A2eSEViduQ2] 获取任务失败: %v", err)
		v.updateTaskStatus(ctx, taskID, "failed", "获取任务状态失败")
		return
	}

	var taskStatus SETaskStatus
	if err := json.Unmarshal([]byte(statusData), &taskStatus); err != nil {
		log.Printf("[A2eSEViduQ2] 解析任务失败: %v", err)
		v.updateTaskStatus(ctx, taskID, "failed", "解析任务状态失败")
		return
	}

	// 步骤1：使用原图生成尾帧图
	log.Printf("[A2eSEViduQ2] 任务 %s - 步骤1：开始生成尾帧图", taskID)
	log.Printf("[A2eSEViduQ2] Image Parameters: %s", taskStatus.ImageParameters)

	// 更新状态为生成尾帧中
	v.updateTaskStatus(ctx, taskID, "generating_tail", "")

	// 直接使用 ImageParameters JSON 字符串作为 A2E 请求体
	payload := []byte(taskStatus.ImageParameters)

	log.Printf("[A2eSEViduQ2] 调用 A2E API: %s", v.a2eGenerateURL)
	log.Printf("[A2eSEViduQ2] 请求 Body: %s", string(payload))

	// 调用 A2E
	a2eResp, err := util.RequestWithClient[a2eSEImageGenerateResponse](
		v.client,
		http.MethodPost,
		v.a2eGenerateURL,
		v.a2eHeaders,
	...
	log.Printf("[A2eSEViduQ2] 任务 %s - 图片任务 ID: %s", taskID, imageTaskID)

	// 轮询尾帧图生成状态
	for {
		tailImageURL := v.pollImageStatus(ctx, taskID, imageTaskID)
		if tailImageURL != "" {
			// 尾帧图生成成功，开始生成视频
			v.startVideoGeneration(ctx, taskID, taskStatus.InputImage, tailImageURL)
			break
		} else if v.isTaskFailed(ctx, taskID) {
			// 尾帧图生成失败
			log.Printf("[A2eSEViduQ2] 任务 %s - 尾帧图生成失败", taskID)
			break
		}
...
func (v *A2eSEViduQ2) pollImageStatus(ctx context.Context, taskID, imageTaskID string) string {
	url := fmt.Sprintf(v.a2eQueryURL, imageTaskID)

	resp, err := util.RequestWithClient[a2eSEImageQueryResponse](
		v.client,
		http.MethodGet,
		url,
		v.a2eHeaders,
	...
	if err != nil {
		log.Printf("[A2eSEViduQ2] 查询 A2E 失败: %v", err)
		return ""
	}

	log.Printf("[A2eSEViduQ2] A2E 图片状态: %s", resp.Data.CurrentStatus)

	if resp.Data.CurrentStatus == "completed" && len(resp.Data.ImageUrls) > 0 {
		tailImageURL := resp.Data.ImageUrls[0]
		log.Printf("[A2eSEViduQ2] 任务 %s - 尾帧图生成完成: %s", taskID, tailImageURL)

		// 保存尾帧图 URL
		v.updateTaskField(ctx, taskID, "tail_image_url", tailImageURL)

		return tailImageURL
	} else if resp.Data.CurrentStatus == "failed" {
		// 图片生成失败，更新任务状态
		v.updateTaskStatus(ctx, taskID, "failed", fmt.Sprintf("尾帧图生成失败: %s", resp.Data.FailedMessage))
	}

	return ""
}

// startVideoGeneration 开始生成视频（首尾帧模式）
func (v *A2eSEViduQ2) startVideoGeneration(ctx context.Context, taskID, headImageURL, tailImageURL string) {
	// 获取任务
	taskKey := fmt.Sprintf("task:%s", taskID)
	...
	log.Printf("[A2eSEViduQ2] 任务 %s - 步骤2：开始生成视频（首尾帧模式）", taskID)
	log.Printf("[A2eSEViduQ2] 首帧（原图）: %s", headImageURL)
	log.Printf("[A2eSEViduQ2] 尾帧（生成）: %s", tailImageURL)
	log.Printf("[A2eSEViduQ2] Video Parameters: %s", taskStatus.VideoParameters)

	// 解析 VideoParameters JSON 字符串
	var viduq2Req viduq2SEVideoGenerateRequest
	...
	log.Printf("[A2eSEViduQ2] 调用 ViduQ2 API: %s", v.viduq2GenerateURL)
	log.Printf("[A2eSEViduQ2] 请求 Body: %s", string(payload))

	// 调用 ViduQ2
	viduq2Resp, err := util.RequestWithClient[viduq2SEVideoGenerateResponse](
		v.client,
		http.MethodPost,
		v.viduq2GenerateURL,
		v.viduq2Headers,
	...
	log.Printf("[A2eSEViduQ2] 任务 %s - 视频任务 ID: %s", taskID, viduq2Resp.Data.JobID)
	...
	for {
		...
		if videoURL != "" {
			...
			// 视频生成失败
			log.Printf("[A2eSEViduQ2] 任务 %s - 视频生成失败", taskID)
			break
		}
...
func (v *A2eSEViduQ2) pollVideoStatus(ctx context.Context, taskID, videoTaskID string) string {
	url := fmt.Sprintf(v.viduq2QueryURL, videoTaskID)

	resp, err := util.RequestWithClient[viduq2SEVideoQueryResponse](
		v.client,
		http.MethodGet,
		url,
		v.viduq2Headers,
	...
	if err != nil {
		log.Printf("[A2eSEViduQ2] 查询 ViduQ2 失败: %v", err)
		return ""
	}

	log.Printf("[A2eSEViduQ2] ViduQ2 视频状态: %d", resp.Data.Status)
	...
	if resp.Data.Status == 2 {
		if len(resp.Data.Result.Videos) > 0 && resp.Data.Result.Videos[0].URL != "" {
			...
			taskStatus.TailImageURL = videoURL // 使用 TailImageURL 字段存储视频 URL
			taskStatus.UpdatedAt = time.Now()
...
func (v *A2eSEViduQ2) updateTaskStatus(ctx context.Context, taskID, status, errorMsg string) {
	taskKey := fmt.Sprintf("task:%s", taskID)
	...
	statusJSON, _ := json.Marshal(taskStatus)
	if err := v.redis.Set(ctx, taskKey, statusJSON, 24*time.Hour).Err(); err != nil {
		log.Printf("[A2eSEViduQ2] 更新任务失败: %v", err)
	}
}

// updateTaskField 更新任务的某个字段
func (v *A2eSEViduQ2) updateTaskField(ctx context.Context, taskID, field, value string) {
	taskKey := fmt.Sprintf("task:%s", taskID)
...
func (v *A2eSEViduQ2) isTaskFailed(ctx context.Context, taskID string) bool {
	taskKey := fmt.Sprintf("task:%s", taskID)
	statusData, err := v.redis.Get(ctx, taskKey).Result()
	if err != nil {
		return false
...
// Query 查询任务状态
func (v *A2eSEViduQ2) Query(taskID string) (*TaskResponse, error) {
	ctx := context.Background()
	taskKey := fmt.Sprintf("task:%s", taskID)
	...
	// 根据状态决定是否需要查询上游
	if taskStatus.Status == "generating_tail" && taskStatus.ImageTaskID != "" {
		// 查询 A2E 尾帧图状态
		tailImageURL := v.pollImageStatus(ctx, taskID, taskStatus.ImageTaskID)
		if tailImageURL != "" {
			// 重新获取任务状态
			statusData, _ = v.redis.Get(ctx, taskKey).Result()
			json.Unmarshal([]byte(statusData), &taskStatus)
	...
	// 如果完成且有视频URL，添加到响应中
	if status == 2 && taskStatus.TailImageURL != "" {
		taskResp.Videos = append(taskResp.Videos, VideoInfo{
			URL: taskStatus.TailImageURL,
		})
	}
